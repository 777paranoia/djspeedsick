  ...
  (function () {
    ...
    ((window.__primeVideoPool = function () {
      ...
      ((e.fixed["files/mov/bh2.mp4"] = [
        ...
        (e.fixed["files/mov/earth.mp4"] = [t("files/mov/earth.mp4", !0)]));
      ...
      (window.__registerVideo = function (t) {
        try {
          ...
          const i = e();
          if (
            (i && t.parentNode !== i && i.appendChild(t), window._siteEntered)
          ) {
            const e = t.play();
            e && e.catch && e.catch(() => {});
          }
...
function warmPrograms() {
  ...
  for (let t = 0; t < e.length; t++) GLSL.modules[e[t]] && buildProgram(e[t]);
}
...
const MAX_VERTEX_ATTRIBS = gl.getParameter(gl.MAX_VERTEX_ATTRIBS) || 8;

function prepareFullscreenDraw(e, t) {
  (gl.disable(gl.DEPTH_TEST),
    gl.disable(gl.CULL_FACE),
    gl.disable(gl.BLEND),
    gl.bindBuffer(gl.ARRAY_BUFFER, t || buf));
  for (let i = 0; i < MAX_VERTEX_ATTRIBS; i++) gl.disableVertexAttribArray(i);
  const i = gl.getAttribLocation(e, "p");
  i >= 0 &&
...
const BACKLIGHT = (() => {
  ...
  return (
    ...
    {
      prog: e,
      Ucol: gl.getUniformLocation(e, "u_col"),
    }
  );
})();
...
function drawBacklight(e, t, i) {
  if (t <= 5e-4) return;
  (gl.useProgram(BACKLIGHT.prog), prepareFullscreenDraw(BACKLIGHT.prog));
  const l = 0.001 * e,
...
class ActiveMode {
  constructor(e) {
    ...
    if (
      ...
      if (
        ...
        IS_MOBILE
          ? (gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv1"), 7),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv2"), 7),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv3"), 7),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv4"), 7),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv6"), 7),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv5"), 7))
          : (gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv1"), 8),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv2"), 9),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv3"), 10),
            gl.uniform1i(gl.getUniformLocation(this.prog, "u_texEnv4"), 11),
        ...
        (this.U = {
          ...
          doorOpen: gl.getUniformLocation(this.prog, "u_doorOpen"),
        }),
        "room_left" === t)
      )
        ((this.env1 = loadStaticTex("files/img/rooms/z1/left-mobile.png")),
        ...
        ((this.doorOpenTex = loadStaticTex("files/img/rooms/z1/door.png")),
          (this.doorClosedTex = loadStaticTex(
            "files/img/rooms/z1/door-closed.png",
          )),
          (this.env1 = window.__z4Route
            ? this.doorOpenTex
            : this.doorClosedTex),
          this.textures.push(this.doorOpenTex, this.doorClosedTex),
        ...
        ((this.env1 = loadStaticTex("files/img/rooms/z1/right-mobile.png")),
          this.textures.push(this.env1),
        ...
        if (
          ...
          ((this.env2 = loadStaticTex(
            window.__mirrorOverlay || e[Math.floor(Math.random() * e.length)],
          )),
          ...
          "goreville" === t
            ...
            : "ocean" === t
              ? (this.env1 = loadStaticTex("files/img/rooms/z1/ocean.png"))
              : "deadcity" === t
  ...
  _makeMappedVideo() {
    const e = window.__claimMappedPoolVid && window.__claimMappedPoolVid();
    if (e) return e;
  ...
  render(e, t, i, n, l, o, r, a, g) {
    ...
    const __deadcityMode2 = 7 === this.id,
      __deadcityBuildings = __deadcityMode2 ? this.bTexs : null;
    (gl.useProgram(this.prog),
      prepareFullscreenDraw(this.prog),
      96 === this.id &&
        (this.env1 = window.__z4Route ? this.doorOpenTex : this.doorClosedTex),
      !IS_MOBILE &&
        this.videoObj &&
      ...
      gl.bindTexture(
        gl.TEXTURE_2D,
        (__deadcityBuildings && __deadcityBuildings[0]) || staticAssets.b1,
      ),
      gl.activeTexture(gl.TEXTURE1),
      gl.bindTexture(
        gl.TEXTURE_2D,
        (__deadcityBuildings && __deadcityBuildings[1]) || staticAssets.b2,
      ),
      gl.activeTexture(gl.TEXTURE2),
      gl.bindTexture(
        gl.TEXTURE_2D,
        (__deadcityBuildings && __deadcityBuildings[2]) || staticAssets.b3,
      ),
      gl.activeTexture(gl.TEXTURE3),
      gl.bindTexture(
        gl.TEXTURE_2D,
        (__deadcityBuildings && __deadcityBuildings[3]) || staticAssets.b4,
      ),
      gl.activeTexture(gl.TEXTURE4),
      gl.bindTexture(
        gl.TEXTURE_2D,
        (__deadcityBuildings && __deadcityBuildings[4]) || staticAssets.b5,
      ),
      gl.activeTexture(gl.TEXTURE5),
      gl.bindTexture(
        gl.TEXTURE_2D,
        (__deadcityBuildings && __deadcityBuildings[5]) || staticAssets.b6,
      ),
      ...
      null !== this.U.doorOpen &&
        gl.uniform1f(this.U.doorOpen, window.__z4Route ? 1 : 0),
      null !== this.U.isOOB && gl.uniform1f(this.U.isOOB, this.isOOB ? 1 : 0),
...
function _laptopTuvToCanvasPx(u, v) {
  ...
  return (
    sa > va
      ? ((uvX = preU), (uvY = (preV - 0.5) / (va / sa) + 0.5))
      : ((uvX = (preU - 0.5) / (sa / va) + 0.5), (uvY = preV)),
    {
      x: uvX * W,
      y: uvY * H,
    }
...
function updateLaptopIframe() {
  if (
    !("laptop" === activePOV && laptopZoom > 0.985) ||
    ("waking" === phase && !laptopScreenAwake)
  )
    return void (
      laptopIframe &&
  ...
  (ensureLaptopIframe(), (laptopIframe.style.pointerEvents = "auto"));
  const tl = _laptopTuvToCanvasPx(LAPTOP_RECT.u0, LAPTOP_RECT.v0),
    br = _laptopTuvToCanvasPx(LAPTOP_RECT.u1, LAPTOP_RECT.v1),
...
function drawHallucinationOverlay(e, t, i, n, l) {
  ...
  const o = void 0 !== n ? n : 0.001 * (e - blinkPeakTime),
    ...
    g = void 0 !== l ? l : 1;
  ((_tripAccum += 8e-5 * r),
    (r < 0.02 && _tripAccum < 0.1) ||
      (gl.useProgram(hallucinationProg),
      prepareFullscreenDraw(hallucinationProg, hallucinationQuadBuf),
      gl.enable(gl.BLEND),
  ...
  if (
    ...
    (backZoomTarget =
      "back" === activePOV && __e1SpaceHeld
        ...
        : 0),
    (backZoom +=
      (backZoomTarget - backZoom) *
      Math.min(1, (__e1SpaceHeld ? 0.06 : 0.18) * i)),
  ...
  if (
    ...
    "center" === activePOV
      ? (gl.bindFramebuffer(gl.FRAMEBUFFER, null),
        ...
        currentEngine &&
          currentEngine.render(e, cx, cy, n, blink, flash, shake, l, modeSeed),
        drawHallucinationOverlay(e))
      : "left" === activePOV
        ? (gl.clearColor(0, 0, 0, 1),
          gl.clear(gl.COLOR_BUFFER_BIT),
          leftEngine &&
            leftEngine.render(e, cx, cy, n, blink, flash, shake, l, modeSeed),
          drawHallucinationOverlay(e))
        : "laptop" === activePOV
          ? (gl.clearColor(0, 0, 0, 1),
            gl.clear(gl.COLOR_BUFFER_BIT),
            laptopEngine &&
              laptopEngine.render(
                ...
                flash,
                shake,
                "waking" === phase ? l : laptopScreenAwake ? 1 : l,
                modeSeed,
              ),
            drawHallucinationOverlay(e))
          : "right" === activePOV &&
    ...
    var backZoom01 = Math.max(0, Math.min(1, backZoom)),
      backEntering = backZoom01 > 0.01,
      backCamX = backEntering
        ? -backZoom01 * backZoom01 * (3 - 2 * backZoom01) * 1.05
        : cx,
      backCamY = backEntering ? 0 : cy,
      backAudio = backEntering ? 0 : n,
      backBlink = backEntering ? 0 : blink,
      backFlash = backEntering ? 0 : flash;
    if (
      (gl.clearColor(0, 0, 0, 1),
      gl.clear(gl.COLOR_BUFFER_BIT),
      simStep(e),
    ...
    backEntering || drawHallucinationOverlay(e);
  }
  if ("door" === activePOV) {
    if (
      (gl.clearColor(0, 0, 0, 1),
      ...
      doorEngine &&
        ...
        (doorZoomTarget =
          __e1SpaceHeld && window.__z4Route
            ? Math.min(1, doorZoomTarget + 0.025 * i)
            : 0),
        (doorZoom +=
          (doorZoomTarget - doorZoom) *
          Math.min(1, (__e1SpaceHeld ? 0.06 : 0.18) * i)),
      ...
      return (
        doorFade ||
          (((doorFade = document.createElement("div")).id =
            "zone-fade-overlay"),
          (doorFade.style.cssText =
            "position:fixed;inset:0;background:black;opacity:0;pointer-events:none;transition:opacity 1.0s ease-in-out;z-index:99999;"),
        ...
        void setTimeout(function () {
          (currentEngine && (currentEngine.destroy(), (currentEngine = null)),
            ...
            (laptopZoom = 0),
            (laptopZoomTarget = 0),
            "function" == typeof window.startZone4 && window.startZone4(),
            setTimeout(function () {
...
window.__wakeToLaptopFromTheater = function () {
  try {
    ...
    ((window.__modeTheaterScene = null),
      ...
      (laptopZoomTarget = 1),
      destroyLaptopIframe(),
      (laptopIframeSrc = "desktop.html?wake=1"),
      (laptopScreenAwake = !0),
      (blinkPeakTime = e));
    try {
      currentEngine && currentEngine.destroy();
    ...
    try {
      leftEngine && leftEngine.destroy();
    } catch (t) {}
    try {
      rightEngine && rightEngine.destroy();
    } catch (t) {}
    try {
      backEngine && backEngine.destroy();
    } catch (t) {}
    try {
      doorEngine && doorEngine.destroy();
    } catch (t) {}
    try {
      laptopEngine && laptopEngine.destroy();
    } catch (t) {}
    ((currentEngine = null),
      (leftEngine = null),
      (rightEngine = null),
    ...
    console.error("[engine1] laptop wake failed", err);
  }
};
