  ...
  ["rss-top", "rss-bottom", "rss-left", "rss-right"].forEach((t, n) => {
    ...
    fetch(
      ...
      .then((e) => {
        let n = [];
        if (
          (e.items && e.items.forEach((e) => n.push(e.title.toUpperCase())),
          0 === n.length)
        )
          return;
...
if (window.innerWidth > 768 && window.innerWidth > window.innerHeight)
  for (
    var splashImgs = bgContainer ? bgContainer.querySelectorAll("img") : [],
      si = 0;
...
const startSplashDrag = (e) => {
    if (
      ...
      "tutorial-link" === e.target.id ||
      e.target.closest("#tutorial-link")
    )
      return;
...
(splashScreen.addEventListener("mousedown", startSplashDrag),
  ...
  splashScreen.addEventListener("touchstart", startSplashDrag, { passive: !1 }),
  ...
  (o ||
    ...
    (o.onload = function () {
      var w = o.naturalWidth || 1,
        h = o.naturalHeight || 1,
        maxW = Math.min(
          window.innerWidth * (i ? 0.985 : 0.96),
          i ? 1800 : 1660,
        ),
        maxH = Math.min(window.innerHeight * (i ? 0.9 : 0.78), i ? 1080 : 900),
    ...
    if (t && n.length > 0) {
      if (!window.__audioSetupDone) {
        ...
        for (let e = 0; e < 2; e++) {
          ...
          for (let e = 0; e < c; e++)
            t[e] =
              (0.7 * (2 * Math.random() - 1) + 0.3 * (2 * Math.random() - 1)) *
              Math.pow(1 - e / c, 2.5) *
      ...
      const e = () => {
        ...
        ((window.__activePlaylist && window.__activePlaylist === e) ||
          ...
          (window.__shuffledTracks = [...e].sort(() => Math.random() - 0.5)),
...
((window.enterSite = function () {
  ...
  (window.startTutorialFromSplash = function (event) {
    if (
      (event && event.preventDefault && event.preventDefault(),
      event && event.stopPropagation && event.stopPropagation(),
      _entered || window.__tutorialRoom)
    )
      return;
    ((_entered = !0), (window._siteEntered = !0));
    const splash = document.getElementById("splash-screen"),
      bg = document.getElementById("background-container"),
      engineCanvas = document.getElementById("c");
    function startTutorial() {
      window.TutorialRoom && window.TutorialRoom.start
        ? window.TutorialRoom.start()
        : console.warn("TutorialRoom did not load.");
    }
    if (
      ...
      return void startTutorial();
    if (document.querySelector('script[data-tutorial-entry="tutorial.js"]'))
      return;
    ((window.TUTORIAL_TEXTURE_ROOT =
    ...
    ((script.src = "tutorial.js?v=20260524-tvframes2"),
      ...
      (script.onerror = function () {
        console.warn("Could not load tutorial.js");
      }),
