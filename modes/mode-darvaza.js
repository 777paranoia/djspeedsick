      ...
      class Zone2Engine {
        constructor() {
          ((this.prog = buildProgram("zone2_hallway")),
            ...
            (this.z2ExitMaskTex = loadStaticTex("files/img/rooms/z2/hallway/canalport-mask.png")),
        ...
        render(e, t, i, o, n, l, r) {
          ...
          if (
            ...
            if (_canWalk && (window.z2SpaceHeld || window.z2TouchHeld)) {
              ...
              if (_f === "S" && this.camZ <= -1.9 && !_seqActive && !this.z2ExitStarted) {
            ...
            (function(self, now) {
              if (!self.darvazaGL) {
                ...
                self.darvazaGL = self.darvazaCanvas.getContext("webgl", {
...
float fbm(vec2 p){float v=0.0,a=0.5;for(int i=0;i<4;i++){v+=a*noise(p);p*=2.2;a*=0.45;}return v;}
float map(vec3 p){float dToC=length(p.xz);float sink=max(0.0,18.0-dToC)*0.85;float rocks=fbm(p.xz*0.5+p.y*0.5)*3.0*smoothstep(18.0,9.0,dToC);return(p.y+fbm(p.xz*0.1)*1.5+sink+rocks)*0.22;}
vec3 getNormal(vec3 p){vec2 e=vec2(0.01,0.0);return normalize(vec3(map(p+e.xyy)-map(p-e.xyy),map(p+e.yxy)-map(p-e.yxy),map(p+e.yyx)-map(p-e.yyx)));}
void main(){
  vec2 uv=(gl_FragCoord.xy-0.5*u_resolution.xy)/u_resolution.y;
  vec3 ro=vec3(0.0,28.0,-28.0);
  vec3 rd=normalize(vec3(uv.x*0.9,uv.y*0.9-0.55,1.0));
  vec3 sky=vec3(0.02,0.01,0.04);
  vec3 col=sky;
  float t=0.0;
  for(int i=0;i<110;i++){float h=map(ro+rd*t);if(abs(h)<0.002||t>180.0)break;t+=h;}
  if(t<180.0){
    vec3 pos=ro+rd*t;
    vec3 norm=getNormal(pos);
    float dToC=length(pos.xz);
    float flicker=1.0+0.2*sin(u_time*22.0)*hash(vec2(u_time));
    vec3 fireCol=vec3(1.0,0.4,0.05);
    float diff=max(0.0,dot(norm,normalize(vec3(-pos.x,5.0,-pos.z))));
    float glow=exp(-dToC*0.15)*flicker;
    col=vec3(0.2,0.14,0.09)*(diff+0.05)+fireCol*glow*2.8;
    if(dToC<18.0){
      float fire=fbm(pos.xz*0.4+u_time*1.5);
      col+=fireCol*fire*smoothstep(2.0,-10.0,pos.y)*10.0*flicker;
      if(pos.y<-3.0)col+=vec3(1.0,0.9,0.5)*exp(-dToC*0.6)*flicker*6.0;
    }
    col=mix(col,sky,smoothstep(70.0,180.0,t));
  }
  gl_FragColor=vec4(pow(col,vec3(0.4545)),1.0);
}`;
                function cs(dgl,type,src){const s=dgl.createShader(type);dgl.shaderSource(s,src);dgl.compileShader(s);if(!dgl.getShaderParameter(s,dgl.COMPILE_STATUS))console.error("[darvaza]",dgl.getShaderInfoLog(s));return s;}
                const prog=dgl.createProgram();
                dgl.attachShader(prog,cs(dgl,dgl.VERTEX_SHADER,vs));
                dgl.attachShader(prog,cs(dgl,dgl.FRAGMENT_SHADER,fs));
                dgl.linkProgram(prog);
                if(!dgl.getProgramParameter(prog,dgl.LINK_STATUS))console.error("[darvaza] link:",dgl.getProgramInfoLog(prog));
                ...
                self.darvazaProg={prog,buf,uRes:dgl.getUniformLocation(prog,"u_resolution"),uTime:dgl.getUniformLocation(prog,"u_time")};
              }
            ...
            const l = m
              ? this.texFront
              : (o && this.texFrontAlt) || this.texFront;
            (gl.useProgram(this.prog),
              gl.activeTexture(gl.TEXTURE0),
              gl.bindTexture(gl.TEXTURE_2D, l),
              gl.activeTexture(gl.TEXTURE1),
              gl.bindTexture(gl.TEXTURE_2D, this.seqState !== "initial" ? this.texBackAlt : this.texBack),
