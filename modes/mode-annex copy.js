(function () {
  ...
  window.Zone4Annex.buildStationMeshes = function () {
    ...
    const INNER_W = -2.35;
    const OUTER_W = 2.35;
    const LEFT_MID_TOP = 0.6;
    const LEFT_MID_BOT = -0.4;
    const RIGHT_MID_TOP = 0.6;
    const RIGHT_MID_BOT = -0.4;
    const LEFT_TOP_X = -1.7;
    const RIGHT_TOP_X = 1.7;
    ...
    function addSection(i) {
      ...
      {
        const floorTex = isGroupedWindowFloorSection(i)
          ? TEX.floorGlass
          : TEX.floor;
        const q = [];
        pushQuad(
      ...
      if (i === 8) {
        ...
        const doorL = [
          fmid.center[0] +
            fmid.radial[0] * INNER_W +
            fmid.tangent[0] * -hallHalfW,
          0,
          fmid.center[2] +
            fmid.radial[2] * INNER_W +
        ...
        const doorL_low = [
          fmid.center[0] +
            fmid.radial[0] * LEFT_LOW_X +
            fmid.tangent[0] * -hallHalfW,
          0,
          fmid.center[2] +
            fmid.radial[2] * LEFT_LOW_X +
            fmid.tangent[2] * -hallHalfW,
        ];
        const doorR_low = [
          fmid.center[0] +
        ...
        const doorL_top = [
          fmid.center[0] +
            fmid.radial[0] * LEFT_TOP_X +
            fmid.tangent[0] * -hallHalfW,
          0,
          fmid.center[2] +
            fmid.radial[2] * LEFT_TOP_X +
            fmid.tangent[2] * -hallHalfW,
        ];
        const doorR_top = [
          fmid.center[0] +
      ...
      }
      {
        ...
        if (rightIsDoor) {
          ...
          function dL(y) {
            return doorPoint(OUTER_W, y, -hallHalfW);
          }
          function dR(y) {
            return doorPoint(OUTER_W, y, hallHalfW);
          }
          function dL_low(y) {
            return doorPoint(RIGHT_LOW_X, y, -hallHalfW);
          }
          function dR_low(y) {
            return doorPoint(RIGHT_LOW_X, y, hallHalfW);
          }
          function dL_top(y) {
            return doorPoint(RIGHT_TOP_X, y, -hallHalfW);
          }
          function dR_top(y) {
            return doorPoint(RIGHT_TOP_X, y, hallHalfW);
          }
        ...
        if (!rightIsDoor) {
          ...
          meshes.push(
            self._makeMesh(q, rightIsWindow ? TEX.winC : TEX.rp2, [1, 1, 1], 1),
          );
        }
    ...
    (function addBay() {
      ...
      pushQuad(
        q,
        pointLocal(cx, cy1 - 0.12, cz0),
        pointLocal(cx, cy1 - 0.12, cz1),
      ...
      for (let p = 0; p < 8; p++) {
        ...
        const gateDelta = Math.atan2(
          Math.sin(a - gateAngle),
          Math.cos(a - gateAngle),
        );
    ...
    (function addHallway() {
      ...
      pushQuad(
        q,
        pointLocal(hx0, hy0, hz0),
        pointLocal(hx0, hy0, hz1),
      ...
      pushQuad(
        q,
        pointLocal(hx0, hy1, hz1),
        pointLocal(hx0, hy1, hz0),
      ...
      pushQuad(
        q,
        pointLocal(hx0, hy0 + 0.01, hz0),
        pointLocal(hx1, hy0 + 0.01, hz0),
        pointLocal(hx1, hy0 + 0.01, hz0 + trimW),
        pointLocal(hx0, hy0 + 0.01, hz0 + trimW),
        [0, 1],
        [1, 1],
        [1, 0],
        [0, 0],
      ...
      pushQuad(
        q,
        pointLocal(hx0, hy0 + 0.01, hz1 - trimW),
        pointLocal(hx1, hy0 + 0.01, hz1 - trimW),
        pointLocal(hx1, hy0 + 0.01, hz1),
        pointLocal(hx0, hy0 + 0.01, hz1),
        [0, 1],
        [1, 1],
        [1, 0],
        [0, 0],
    ...
    (function addStationAnnex() {
      ...
      function addRoom(xa, xb, ya, yb, za, zb, openA, openB) {
        ...
        const wallDim = [0.035, 0.04, 0.045];
        const floorDim = [0.05, 0.055, 0.06];
        const ceilDim = [0.025, 0.03, 0.035];
        addTiledPanel(
          pointLocal(xa, ya, za),
          pointLocal(xa, ya, zb),
          pointLocal(xb, ya, zb),
      ...
      function addSpeakerStage(x0, x1, floorY, topY, zBack, zFront) {
        ...
        const faceZ = zBack + 0.5;
        const stageX0 = x0 + 0.76;
        const stageX1 = x1 + 0.28;
        const dark = [0.01, 0.011, 0.011];
        addBox(
          stageX0,
          stageX1,
          floorY + 0.02,
      ...
      function addRaveAftermath(x0, x1, y, z0, z1) {
        const bits = [
          ...
          [0.47, 0.48, 0.12, 0.12, 0.0, 0.7, 0.95, 0.2],
        ];
      ...
      function addAnnexCrowdPlane(tex, cx, cz, h, w, facing) {
        ...
        mesh.crowdDanceSeed =
          Math.sin((cx + 19.13) * 17.17 + (cz - 3.7) * 31.31) * 43758.5453;
        mesh.crowdDanceAmp = 0.7 + (Math.abs(mesh.crowdDanceSeed) % 1.0) * 0.46;
        meshes.push(mesh);
        return mesh;
      }
      function addSteepStairwell(xa, xb, topY0, topY1, botY0, botY1, halfW) {
        ...
        const wallDim = [0.03, 0.034, 0.038];
        const treadDim = [0.042, 0.046, 0.05];
      ...
      function addStairwellCrackSeal(
        xa,
        xb,
        topY0,
        topY1,
        ...
        const dim = [0.026, 0.03, 0.034];
        addPanel(
          pointLocal(xa - 0.035, topY0 - 0.03, z0),
          pointLocal(xb + 0.055, botY0 - 0.03, z0),
          pointLocal(xb + 0.055, botY1 + 0.07, z0),
          pointLocal(xa - 0.035, topY1 + 0.07, z0),
          TEX.stairWall,
          dim,
          0.74,
        );
        addPanel(
          ...
          pointLocal(xa - 0.035, topY0 - 0.03, z1),
          pointLocal(xa - 0.035, topY1 + 0.07, z1),
        ...
        addPanel(
          pointLocal(xa - 0.035, topY0 - 0.06, z0),
          pointLocal(xa - 0.035, topY0 - 0.06, z1),
      ...
      const stairX1 = stairX0 + 7.8;
      const stairHalfW = 0.38;
      const annexDrop = 3.8;
      const annexX0 = stairX1;
      const annexX1 = annexX0 + 7.8;
      ...
      for (let mi = topStairDoorBulbStart; mi < meshes.length; mi++)
        meshes[mi].annexTopDoorBulb = true;
      ...
      const normalBasementStart = meshes.length;
      {
        ...
        const exitTilesY = 1;
        addTiledPanel(
          pointLocal(annexX1 - 0.03, annexY0, -annexHalfW),
          pointLocal(annexX1 - 0.03, annexY0, annexHalfW),
          pointLocal(annexX1 - 0.03, annexY1, annexHalfW),
          pointLocal(annexX1 - 0.03, annexY1, -annexHalfW),
          TEX.basementWall,
          [0.035, 0.04, 0.045],
          0.58,
        ...
        for (let mi = baseStart; mi < meshes.length; mi++)
          meshes[mi].annexBaseExitWall = true;
      }
      ...
      self.annexLightWorld = [
        pointLocal(annexX0 + 2.45, annexY1 - 0.14, -annexHalfW + 1.1),
        pointLocal(annexX0 + 0.78, annexY1 - 0.14, 0.0),
      ...
      addCeilingBulb(annexX0 + 2.45, -annexHalfW + 1.1, annexY0, annexY1);
      addCeilingBulb(annexX0 + 0.78, 0.0, annexY0, annexY1);
      addBulkheadWithOpening(
        annexX0 + 0.01,
        annexY0,
        annexY1,
      ...
      );
      {
        ...
        const backRows = [
          {
            z: -3.1,
            xs: [
              0.56, 0.98, 1.42, 1.88, 2.36, 2.84, 3.32, 3.8, 4.28, 4.76, 5.24,
              5.7, 6.14, 6.52,
            ],
            h: 1.52,
            w: 0.7,
          },
          {
            z: -2.78,
            xs: [
              0.72, 1.18, 1.66, 2.14, 2.62, 3.1, 3.58, 4.06, 4.54, 5.02, 5.5,
              5.98, 6.42,
            ],
            h: 1.5,
            w: 0.7,
          ...
          },
          {
            z: -1.74,
            xs: [
              0.58, 1.04, 1.54, 2.04, 2.54, 3.04, 3.54, 4.04, 4.54, 5.04, 5.54,
              6.04, 6.46,
            ],
            h: 1.4,
            w: 0.62,
          },
          {
            z: -1.38,
            xs: [
              0.86, 1.34, 1.86, 2.38, 2.9, 3.42, 3.94, 4.46, 4.98, 5.5, 6.02,
            ],
            h: 1.36,
            w: 0.6,
          },
        ...
        for (let r = 0; r < backRows.length; r++) {
          ...
          for (let k = 0; k < row.xs.length; k++) {
            const n = r * 11 + k * 7;
            const jitterX =
              ((Math.sin(n * 12.9898) * 43758.5453) % 1.0) * 0.16 - 0.08;
            const jitterZ =
              ((Math.sin((n + 4.7) * 78.233) * 43758.5453) % 1.0) * 0.12 - 0.06;
            const scale =
              0.92 + ((Math.sin((n + 9.1) * 37.719) * 43758.5453) % 1.0) * 0.16;
            addAnnexCrowdPlane(
        ...
        for (let r = 0; r < sideRows.length; r++) {
          ...
          for (let k = 0; k < row.n; k++) {
            ...
            const jitterX =
              ((Math.sin(n * 19.17) * 43758.5453) % 1.0) * 0.18 - 0.09;
            const jitterZ =
              ((Math.sin((n + 2.3) * 31.43) * 43758.5453) % 1.0) * 0.16 - 0.08;
            const scale =
              0.88 + ((Math.sin((n + 6.6) * 11.91) * 43758.5453) % 1.0) * 0.18;
            addAnnexCrowdPlane(
      ...
      if (
        ...
        typeof window.Zone4AltAnnex.buildAltAnnexScene !== "function"
      ) {
