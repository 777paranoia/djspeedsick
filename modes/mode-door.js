...
vec4 getScreenCol(vec2 tuv) {
    ...
    // Detect doorway: alpha transparency OR green screen OR near-black
    ...
    if (!isDoorway) {
        // Near-black fallback (for JPEG-stripped alpha)
    ...
    if (isDoorway) {
        ...
        // Subtle parallax
        float depthFactor = 0.04 + u_zoom * 0.08;
        ...
        vec3 platCol = texture2D(u_texEnv2, pUV).rgb;

        // Dark it way down — looking through a doorway into a dim space
        platCol *= 0.25 + u_zoom * 0.15;

        return vec4(platCol, 1.0);
    }
...
void main() {
    ...
    // Drift camera toward doorway as spacebar is held
