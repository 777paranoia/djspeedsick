...
uniform sampler2D u_texB1, u_texB2, u_texB3;
#ifdef MOBILE
#define u_texB4 u_texB1
#define u_texB5 u_texB2
#define u_texB6 u_texB3
uniform sampler2D u_water;
uniform sampler2D u_texWindow;
uniform sampler2D u_texEnv1;
#define u_texEnv2 u_texEnv1
#else
uniform sampler2D u_texB4, u_texB5, u_texB6;
...
#ifdef MOBILE
float sdFractal(vec3 p, float power, float rotA, float rotB){ return 99.0; }
#else
...
#endif
#ifdef MOBILE
vec3 colorPickover(vec3 p) { return neonPalette(p.x * 0.3 + u_time * 0.1) * 0.6; }
#else
...
#endif
#ifdef MOBILE
vec3 colorClifford(vec3 p) { return neonPalette(p.y * 0.3 + u_time * 0.08) * 0.5; }
#else
...
void setupCamera(out vec3 ro, out vec3 rd, out vec3 clean_rd, float intensity) {
  ...
  float liqAmp = mix(0.035, 0.085, u_isOOB) * min(u_trip, 2.10);
  uv += (r - 0.5) * liqAmp; 
  float boothTripHard = smoothstep(1.55, 2.20, u_trip);
  uv += vec2(sin(u_time * 0.62 + q.x * 3.0), cos(u_time * 0.47 + q.y * 3.6)) * boothTripHard * 0.010;
  uv *= 1.0 + boothTripHard * (0.010 + 0.006 * sin(u_time * 0.39));
...
vec3 applyHallucination(vec3 baseCol, vec2 screenUV, float fractalSeed, float blinkAge, float trip, float time){
    ...
    float boothTripHard = smoothstep(1.25, 1.90, trip);
    float periph = smoothstep(0.46, 1.16, r);
    float surge = smoothstep(4.5, 0.0, blinkAge) * (0.10 + boothTripHard * 0.10);
    float strength = min(0.24, (min(trip, 1.9) * 0.050 + surge + boothTripHard * 0.055) * periph);
    if(strength < 0.005) return baseCol;
    float typeRoll = _hh1(fractalSeed * 3.7);
    float zoom = mix(0.85, 3.10, _hh1(fractalSeed * 1.3));
    vec2 drift = vec2(sin(time*0.028+fractalSeed)*0.12, cos(time*0.021+fractalSeed*1.7)*0.12);
    drift += vec2(sin(time * 0.13 + fractalSeed), cos(time * 0.11 + fractalSeed * 1.7)) * boothTripHard * 0.065;
    ...
    float grain = (_hh2(screenUV * mix(320.0, 520.0, boothTripHard) + floor(time * 18.0) * 7.3) - 0.5) * min(trip, 2.1) * (0.028 + boothTripHard * 0.018);
    float vignette = smoothstep(0.48, 1.16, r) * (min(trip, 2.1) * 0.045 + boothTripHard * 0.070) * (0.70 + 0.30 * sin(time * 0.34));
    vec3 result = baseCol;
